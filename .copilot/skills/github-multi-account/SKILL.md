...
Each repo is bound to one account. Agents should check the current default `gh auth status` and use `ghp` for personal repos and `ghw` for work repos accordingly.

## For Squad Agents
At the TOP of any script touching GitHub, define (replacing with the correct account usernames for this environment):
```powershell
function gh-personal { gh auth switch --user [personal-account] 2>$null | Out-Null; gh @args }
function gh-work { gh auth switch --user [work-account] 2>$null | Out-Null; gh @args }
```
